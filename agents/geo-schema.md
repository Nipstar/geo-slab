...
>
> **WIKIDATA DATA — USE FOR EXTERNAL-ID SAMEAS.** The orchestrator passes `wikidata_data` from `reports/<domain>/wikidata.json`. When `wikidata.found == true`, the entity already has structured external identifiers (Companies House, Crunchbase, LinkedIn org ID, X handle, etc. — see `external_ids`). Recommend adding the corresponding Wikidata entity URL itself to the on-site Organization sameAs (the canonical link back to Wikidata is one of the strongest entity-verification signals).
>
> **SERPAPI DATA — USE FOR REVIEW/AGGREGATERATING DECISIONS.** The orchestrator passes `serpapi_data` from `reports/<domain>/serpapi.json`. If `summary.review_directories` contains hits (Trustpilot, G2, Clutch, etc.) those reviews exist — recommend a Review/AggregateRating block on the site that mirrors them. If `queries.brand_serp.knowledge_panel.rating` is present, the source is live and citeable in schema.
